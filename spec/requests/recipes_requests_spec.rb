...
RSpec.describe RecipesController, type: :request do
  let(:user) { User.new(name: 'Isabella', email: 'bb@test.com', password: 'password') }
  ...
  describe 'GET #show' do
    let(:recipe) do
      Recipe.create(name: 'Soup', preparation_time: 30, cooking_time: 30, description: 'Soup', public: false,
                    user_id: user.id)
    end
  ...
  describe 'GET #toggle_public' do
    let!(:recipe) do
      Recipe.create(name: 'rice', preparation_time: 10, cooking_time: 20, description: 'Fried rice', public: false,
                    user_id: user.id)
    end
  end
